...
use crate::msg::{ExecuteMsg, InstantiateMarketingInfo, InstantiateMsg, QueryMsg};
use crate::query::{query_balance, query_minter, query_token_info};
...
mod instantiate {
    ...
    mod marketing {
        use super::*;
        use crate::query::{query_download_logo, query_marketing_info};
...
fn can_mint_by_minter() {
    ...
    let info = message_info(&Addr::unchecked(minter.clone()), &[]);
    let env = mock_env();
...
fn transfer() {
    ...
    let info = message_info(&Addr::unchecked(addr2.clone()), &[]);
    let env = mock_env();
    let msg = ExecuteMsg::Transfer {
...
mod migration {
    ...
    use cw20::{
        AllAllowancesResponse, AllSpenderAllowancesResponse, Cw20Coin, SpenderAllowanceInfo,
    };
...
mod marketing {
    ...
    use cw20::{DownloadLogoResponse, EmbeddedLogo, Logo, LogoInfo, MarketingInfoResponse};
