...
abstract contract LidoInteractor is Initializable {
  ...
  function _lidoStake(uint256 ethAmount) internal returns (uint256 stEthAmount) {
    ...
    uint256 lidoTotalShares = IStETH(data.stETH).getTotalShares();
    uint256 lidoTotalPooledEther = IStETH(data.stETH).getTotalPooledEther();

    stEthAmount = Math.mulDiv(lidoShares, lidoTotalPooledEther, lidoTotalShares);
  }
