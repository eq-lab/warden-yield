...
abstract contract LidoInteractor is Initializable {
  ...
  function _lidoFullWithdraw() internal {
    ...
    uint256 stEthAmount = IERC20(data.stETH).balanceOf(address(this));
    ...
    _enqueue(_getLidoWithdrawQueueStorage(), requestIds, amounts);
  ...
  function _lidoCompleteWithdrawal() internal returns (uint256 ethReceived) {
    LidoWithdrawQueue storage withdrawQueue = _getLidoWithdrawQueueStorage();
    uint128 queueStart = withdrawQueue.start;
    if (queueStart == withdrawQueue.end) revert Errors.NoActiveWithdrawal();
  ...
  function _enqueue(LidoWithdrawQueue storage queue, uint256[] memory requestIds, uint256[] memory amounts) private {
    ...
    unchecked {
      for (uint128 i; i < length; ++i) {
        ...
        queue.items[index] = LidoWithdrawQueueElement({requestId: requestIds[i], requested: amounts[i]});
      }
      queue.end += length;
    }
  }
  ...
  function hasPendingLidoWithdrawal() public view returns (bool) {
    LidoWithdrawQueue storage queue = _getLidoWithdrawQueueStorage();
    return queue.start != queue.end;
  }

  function hasClaimableLidoWithdrawal() public view returns (bool) {
    LidoWithdrawQueue storage queue = _getLidoWithdrawQueueStorage();
    uint128 start = queue.start;
    if (start == queue.end) return false;

    LidoWithdrawQueueElement storage elem = queue.items[start];

    uint256[] memory requestIds = new uint256[](1);
    requestIds[0] = elem.requestId;

    address lidoWithdrawalQueue = _getLidoInteractorDataStorage().lidoWithdrawalQueue;
    ILidoWithdrawalQueue.WithdrawalRequestStatus memory status = ILidoWithdrawalQueue(lidoWithdrawalQueue)
      .getWithdrawalStatus(requestIds)[0];
